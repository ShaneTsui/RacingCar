...
def long_term_MPC(ego_car: Car, route, dt, N):
    x, y, v, theta, accel, steer = ego_car.get_car_state()
    ...
    # vars upper bound
    ub_constrains_x = np.array([x] + [INF] * (N - 1))
    ub_constrains_y = np.array([y] + [INF] * (N - 1))
    ub_constrains_theta = np.array([theta] + [INF] * (N - 1))
    ub_constrains_v = np.array([v] + [MAX_v] * (N - 1))
    ub_constrains_steer = np.array([steer] + [MAX_steer] * (N - 2))
    ub_constrains_a = np.array([a] + [MAX_a] * (N - 2))
    ...
    # vars lower bound
    lb_constrains_x = np.array([x] + [-INF] * (N - 1))
    lb_constrains_y = np.array([y] + [-INF] * (N - 1))
    lb_constrains_theta = np.array([theta] + [-INF] * (N - 1))
    lb_constrains_v = np.array([v] + [-MIN_v] * (N - 1))
    lb_constrains_steer = np.array([steer] + [MIN_steer] * (N - 2))
    lb_constrains_a = np.array([a] + [MIN_a] * (N - 2))
